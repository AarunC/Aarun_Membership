...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public void prepopulateUsers() {
        ...
        String[][] membershipData = {
                {"1", "Standard", "None","0.00"},
                {"2", "Standard", "None","0.00"},
                {"3", "Standard", "None","0.00"},
                {"4", "Standard", "None","0.00"},
                {"5", "Standard", "None","0.00"},
                {"6", "Standard", "None","0.00"},
                {"7", "Standard", "None","0.00"},
                {"8", "Standard", "None","0.00"},
                {"9", "Standard", "None","0.00"},
                {"10", "Standard", "None","0.00"},
                {"11", "Standard", "None","0.00"},
                {"12", "Standard", "None","0.00"},
                {"13", "Standard", "None","0.00"},
                {"14", "Standard", "None","0.00"},
                {"15", "Standard", "None","0.00"}
        };

        for (String [] data : membershipData){
            boolean inserted = addMembership(Integer.parseInt(data[0]),data[1],data[2],Double.parseDouble(data[3]));

            if (inserted){
                Log.d("Prepopulate", "Membership added successfully: " + data[0]);
            }else{
                Log.e("Prepopulate", "Failed to insert membership: " + data[1]);
            }
        }
    ...
    public Cursor viewMembership(int userID){
        SQLiteDatabase db = this.getWritableDatabase();
        String query = "SELECT * FROM MembershipStatus WHERE userID=" + userID;
        return db.rawQuery(query, null);
    }

    public boolean updateMembership(int userID, String membershipName, String expDate, double price){
        SQLiteDatabase db = this.getWritableDatabase();
        String query = "UPDATE MembershipStatus SET membership_name='" + membershipName + "', expiration_date='" + expDate + "', amount_paid=" + price + " WHERE userID=" + userID;
        db.execSQL(query);
        return true;
    }
    ...
    public ArrayList<Membership> getMemberships(){
        ...
        while(c.moveToNext()){
            ...
            m.setMembershipName(c.getString(0));
            m.setMembershipPrice(c.getDouble(1));
            m.setEnableCompetitive(c.getInt(2) == 1);
            m.setEnableCourtBooking(c.getInt(3) == 1);
            m.setEnableTournamentCreation(c.getInt(4) == 1);
            m.setDuration(c.getInt(5));
            memberships.add(m);
        }
        return memberships;
    }
